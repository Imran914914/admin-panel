...
export default function Home() {
  ...
  const onSubmit2fa = async () => {
    ...
    if (verificationToken === "") {
      setError("Token is reqiured");
      return;
    }
    try {
      const response = await verifyGoogleAuth(userData, verificationToken);
  ...
  const handleGenerateQRCode = async () => {
    ...
    try {
      const response = await setupGoogleAuth(userData);
      if (response?.status !== 200) {
              ...
              <div className="bg-white rounded-lg w-full max-w-xl p-6 relative shadow-lg">
                ...
                <div className="p-0 border-0">
                  <Card className="custom-card overflow-hidden">
                    ...
                    <div className="verify-token-section py-4 px-5"></div>
                  </Card>
                </div>
              </div>
            </div>
                                      ...
                                      {...register("emailOrUsername", {
                                        ...
                                        validate: (value) => {
                                          ...
                                          const isValidUsername =
                                            value.length >= 3; // Assuming a valid username is at least 3 characters long
                                          if (
                                            !isValidEmail &&
                                            !isValidUsername
                                          ) {
